...
def create_or_update_skillset(
    ...
    try:
        ...
        indexer_client = SearchIndexerClient(ai_search_uri, credential=credentials, api_version=AI_SEARCH_API_VERSION)

        # read definition from the file and replace placeholders with actual values
        definition = _prepare_json_schema(
            skillset_file,
            {"<search_index_name>": index_name, "<skillset_name>": skillset_name, "<open_ai_uri>": open_ai_uri},
        )
...
def create_or_update_indexer(
    ...
    # Create a search indexer client
    try:
        indexer_client = SearchIndexerClient(ai_search_uri, credential=credential, api_version=AI_SEARCH_API_VERSION)
...
def create_or_update_datasource(
    ...
    try:
        ...
        conn_string = _get_storage_conn_string(subscription_id, storage_account_name, resource_group_name)
...
def create_or_update_index(
    ...
    """
    try:
        index_client = SearchIndexClient(ai_search_uri, credential=credential, api_version=AI_SEARCH_API_VERSION)
